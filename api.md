...
```sh
scalarapi registry update-api-document 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi registry delete-api-document 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi registry retrieve-api-document-version 'namespace_' 'slug' 'semver' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi registry update-api-document-version 'namespace_' 'slug' 'semver' --bearer-auth "$BEARER_AUTH" --document 'document'
```
...
```sh
scalarapi registry delete-api-document-version 'namespace_' 'slug' 'semver' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi registry list-api-document-version-metadata 'namespace_' 'slug' 'semver' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi registry create-api-document-version 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --version-command 'version' --document 'document'
```
...
```sh
scalarapi registry create-api-document-access-group 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --access-group-slug 'accessGroupSlug'
```
...
```sh
scalarapi registry delete-api-document-access-group 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --access-group-slug 'accessGroupSlug'
```
...
```sh
scalarapi schemas update 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi schemas delete 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi schemas retrieve-version 'namespace_' 'slug' 'semver' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi schemas delete-version 'namespace_' 'slug' 'semver' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi schemas create-version 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --version-command 'version' --document 'document'
```
...
```sh
scalarapi schemas create-access-group 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --access-group-slug 'accessGroupSlug'
```
...
```sh
scalarapi schemas delete-access-group 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --access-group-slug 'accessGroupSlug'
```
...
```sh
scalarapi rules update-ruleset 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi rules delete-ruleset 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi rules retrieve-ruleset-document 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH"
```
...
```sh
scalarapi rules create-ruleset-access-group 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --access-group-slug 'accessGroupSlug'
```
...
```sh
scalarapi rules delete-ruleset-access-group 'namespace_' 'slug' --bearer-auth "$BEARER_AUTH" --access-group-slug 'accessGroupSlug'
```
